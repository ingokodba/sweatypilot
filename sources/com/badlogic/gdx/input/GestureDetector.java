...
public class GestureDetector extends InputAdapter {
    ...
    class C00621 extends Task {
        C00621() {
        }

        public void run() {
            if (!GestureDetector.this.longPressFired) {
                GestureDetector.this.longPressFired = GestureDetector.this.listener.longPress(GestureDetector.this.pointer1.f102x, GestureDetector.this.pointer1.f103y);
            }
        }
    }
    ...
    public boolean touchUp(float x, float y, int pointer, int button) {
        ...
        if (this.inTapSquare) {
            ...
            this.panning = true;
            if (pointer == 0) {
                this.tracker.start(this.pointer2.f102x, this.pointer2.f103y, Gdx.input.getCurrentEventTime());
            } else {
                this.tracker.start(this.pointer1.f102x, this.pointer1.f103y, Gdx.input.getCurrentEventTime());
            }
            return false;
        } else {
