...
public class TextureRegion {
    ...
    public void setRegion(TextureRegion region) {
        ...
        setRegion(region.f51u, region.f52v, region.u2, region.v2);
    }
    ...
    public void setU2(float u2) {
        this.u2 = u2;
        this.regionWidth = Math.round(Math.abs(u2 - this.f51u) * ((float) this.texture.getWidth()));
    }
    ...
    public void setV2(float v2) {
        this.v2 = v2;
        this.regionHeight = Math.round(Math.abs(v2 - this.f52v) * ((float) this.texture.getHeight()));
    }

    public int getRegionX() {
        return Math.round(this.f51u * ((float) this.texture.getWidth()));
    }
    ...
    public int getRegionY() {
        return Math.round(this.f52v * ((float) this.texture.getHeight()));
    }
    ...
    public void setRegionWidth(int width) {
        if (isFlipX()) {
            ...
            setU2(this.f51u + (((float) width) / ((float) this.texture.getWidth())));
        }
    }
    ...
    public void setRegionHeight(int height) {
        if (isFlipY()) {
            ...
            setV2(this.f52v + (((float) height) / ((float) this.texture.getHeight())));
        }
    }

    public void flip(boolean x, boolean y) {
        if (x) {
            float temp = this.f51u;
            this.f51u = this.u2;
            this.u2 = temp;
        }
        if (y) {
    ...
    public boolean isFlipX() {
        return this.f51u > this.u2;
    }

    public boolean isFlipY() {
        return this.f52v > this.v2;
    }

    public void scroll(float xAmount, float yAmount) {
        if (xAmount != 0.0f) {
            float width = (this.u2 - this.f51u) * ((float) this.texture.getWidth());
            this.f51u = (this.f51u + xAmount) % 1.0f;
            this.u2 = this.f51u + (width / ((float) this.texture.getWidth()));
        }
        if (yAmount != 0.0f) {
            float height = (this.v2 - this.f52v) * ((float) this.texture.getHeight());
            this.f52v = (this.f52v + yAmount) % 1.0f;
            this.v2 = this.f52v + (height / ((float) this.texture.getHeight()));
        }
    }
