...
public class MeshBuilder implements MeshPartBuilder {
    ...
    private static final Pool<Matrix4> matrices4Pool = new C00612();
    ...
    private static final Pool<Vector3> vectorPool = new C00601();
    ...
    static class C00601 extends Pool<Vector3> {
        C00601() {
        }
    ...
    static class C00612 extends Pool<Matrix4> {
        C00612() {
        }
    ...
    private Vector3 tmp(Vector3 copyFrom) {
        return tmp(copyFrom.f107x, copyFrom.f108y, copyFrom.f109z);
    }
    ...
    private static final void transformPosition(float[] values, int offset, int size, Matrix4 transform) {
        if (size > 2) {
            ...
            values[offset] = vTmp.f107x;
            values[offset + 1] = vTmp.f108y;
            values[offset + 2] = vTmp.f109z;
        } else if (size > 1) {
            vTmp.set(values[offset], values[offset + 1], 0.0f).mul(transform);
            values[offset] = vTmp.f107x;
            values[offset + 1] = vTmp.f108y;
        } else {
            values[offset] = vTmp.set(values[offset], 0.0f, 0.0f).mul(transform).f107x;
        }
    }
    ...
    private static final void transformNormal(float[] values, int offset, int size, Matrix3 transform) {
        if (size > 2) {
            ...
            values[offset] = vTmp.set(values[offset], 0.0f, 0.0f).mul(transform).nor().f107x;
        }
    }
    ...
    private final void addVertex(float[] values, int offset) {
        ...
        if (this.hasColor) {
            if (this.colOffset >= 0) {
                ...
                fArr[i] = fArr[i] * this.color.f39r;
                fArr = this.vertices.items;
                i = (this.colOffset + o) + 1;
                fArr[i] = fArr[i] * this.color.f38g;
                fArr = this.vertices.items;
                i = (this.colOffset + o) + 2;
                fArr[i] = fArr[i] * this.color.f37b;
                if (this.colSize > 3) {
                    fArr = this.vertices.items;
                    i = (this.colOffset + o) + 3;
                    fArr[i] = fArr[i] * this.color.f36a;
                }
    ...
    public short vertex(Vector3 pos, Vector3 nor, Color col, Vector2 uv) {
        ...
        this.vertex[this.posOffset] = pos.f107x;
        if (this.posSize > 1) {
            this.vertex[this.posOffset + 1] = pos.f108y;
        }
        if (this.posSize > 2) {
            this.vertex[this.posOffset + 2] = pos.f109z;
        }
        if (this.norOffset >= 0) {
            if (nor == null) {
                nor = this.tmpNormal.set(pos).nor();
            }
            this.vertex[this.norOffset] = nor.f107x;
            this.vertex[this.norOffset + 1] = nor.f108y;
            this.vertex[this.norOffset + 2] = nor.f109z;
        }
        if (this.colOffset >= 0) {
            if (col == null) {
                col = Color.WHITE;
            }
            this.vertex[this.colOffset] = col.f39r;
            this.vertex[this.colOffset + 1] = col.f38g;
            this.vertex[this.colOffset + 2] = col.f37b;
            if (this.colSize > 3) {
                this.vertex[this.colOffset + 3] = col.f36a;
            }
        } else if (this.cpOffset > 0) {
            if (col == null) {
                col = Color.WHITE;
        ...
        if (uv != null && this.uvOffset >= 0) {
            this.vertex[this.uvOffset] = uv.f102x;
            this.vertex[this.uvOffset + 1] = uv.f103y;
        }
    ...
    public void circle(float radius, int divisions, Vector3 center, Vector3 normal) {
        circle(radius, divisions, center.f107x, center.f108y, center.f109z, normal.f107x, normal.f108y, normal.f109z);
    }

    public void circle(float radius, int divisions, Vector3 center, Vector3 normal, Vector3 tangent, Vector3 binormal) {
        circle(radius, divisions, center.f107x, center.f108y, center.f109z, normal.f107x, normal.f108y, normal.f109z, tangent.f107x, tangent.f108y, tangent.f109z, binormal.f107x, binormal.f108y, binormal.f109z);
    }
    ...
    public void circle(float radius, int divisions, Vector3 center, Vector3 normal, float angleFrom, float angleTo) {
        circle(radius, divisions, center.f107x, center.f108y, center.f109z, normal.f107x, normal.f108y, normal.f109z, angleFrom, angleTo);
    }
    ...
    public void circle(float radius, int divisions, Vector3 center, Vector3 normal, Vector3 tangent, Vector3 binormal, float angleFrom, float angleTo) {
        circle(radius, divisions, center.f107x, center.f108y, center.f109z, normal.f107x, normal.f108y, normal.f109z, tangent.f107x, tangent.f108y, tangent.f109z, binormal.f107x, binormal.f108y, binormal.f109z, angleFrom, angleTo);
    }
    ...
    public void ellipse(float width, float height, int divisions, Vector3 center, Vector3 normal) {
        ellipse(width, height, divisions, center.f107x, center.f108y, center.f109z, normal.f107x, normal.f108y, normal.f109z);
    }
    ...
    public void ellipse(float width, float height, int divisions, Vector3 center, Vector3 normal, Vector3 tangent, Vector3 binormal) {
        ellipse(width, height, divisions, center.f107x, center.f108y, center.f109z, normal.f107x, normal.f108y, normal.f109z, tangent.f107x, tangent.f108y, tangent.f109z, binormal.f107x, binormal.f108y, binormal.f109z);
    }
    ...
    public void ellipse(float width, float height, int divisions, Vector3 center, Vector3 normal, float angleFrom, float angleTo) {
        ellipse(width, height, 0.0f, 0.0f, divisions, center.f107x, center.f108y, center.f109z, normal.f107x, normal.f108y, normal.f109z, angleFrom, angleTo);
    }
    ...
    public void ellipse(float width, float height, int divisions, Vector3 center, Vector3 normal, Vector3 tangent, Vector3 binormal, float angleFrom, float angleTo) {
        ellipse(width, height, 0.0f, 0.0f, divisions, center.f107x, center.f108y, center.f109z, normal.f107x, normal.f108y, normal.f109z, tangent.f107x, tangent.f108y, tangent.f109z, binormal.f107x, binormal.f108y, binormal.f109z, angleFrom, angleTo);
    }
    ...
    public void ellipse(float width, float height, float innerWidth, float innerHeight, int divisions, Vector3 center, Vector3 normal) {
        ellipse(width, height, innerWidth, innerHeight, divisions, center.f107x, center.f108y, center.f109z, normal.f107x, normal.f108y, normal.f109z, 0.0f, 360.0f);
    }
    ...
    public void ellipse(float width, float height, float innerWidth, float innerHeight, int divisions, float centerX, float centerY, float centerZ, float normalX, float normalY, float normalZ, float angleFrom, float angleTo) {
        ...
        ellipse(width, height, innerWidth, innerHeight, divisions, centerX, centerY, centerZ, normalX, normalY, normalZ, this.tempV1.f107x, this.tempV1.f108y, this.tempV1.f109z, this.tempV2.f107x, this.tempV2.f108y, this.tempV2.f109z, angleFrom, angleTo);
    }
    ...
    public void ellipse(float width, float height, float innerWidth, float innerHeight, int divisions, float centerX, float centerY, float centerZ, float normalX, float normalY, float normalZ, float tangentX, float tangentY, float tangentZ, float binormalX, float binormalY, float binormalZ, float angleFrom, float angleTo) {
        ...
        for (int i = 0; i <= divisions; i++) {
            ...
            currEx.position.set(centerX, centerY, centerZ).add((sxEx.f107x * x) + (syEx.f107x * y), (sxEx.f108y * x) + (syEx.f108y * y), (sxEx.f109z * x) + (syEx.f109z * y));
            ...
            if (innerWidth <= 0.0f || innerHeight <= 0.0f) {
                ...
                currIn.position.set(centerX, centerY, centerZ).add((sxIn.f107x * x) + (syIn.f107x * y), (sxIn.f108y * x) + (syIn.f108y * y), (sxIn.f109z * x) + (syIn.f109z * y));
    ...
    public void arrow(float x1, float y1, float z1, float x2, float y2, float z2, float capLength, float stemThickness, int divisions) {
        ...
        Vector3 forward = tmp(up).crs(Vector3.f106Z);
        if (forward.isZero()) {
            forward.set(Vector3.f104X);
        }
        forward.crs(up).nor();
