...
public final class GeometryUtils {
    ...
    public static Vector2 toBarycoord(Vector2 p, Vector2 a, Vector2 b, Vector2 c, Vector2 barycentricOut) {
        ...
        barycentricOut.f102x = ((d11 * d20) - (d01 * d21)) / denom;
        barycentricOut.f103y = ((d00 * d21) - (d01 * d20)) / denom;
        return barycentricOut;
    }

    public static boolean barycoordInsideTriangle(Vector2 barycentric) {
        return barycentric.f102x >= 0.0f && barycentric.f103y >= 0.0f && barycentric.f102x + barycentric.f103y <= 1.0f;
    }

    public static Vector2 fromBarycoord(Vector2 barycentric, Vector2 a, Vector2 b, Vector2 c, Vector2 interpolatedOut) {
        float u = (1.0f - barycentric.f102x) - barycentric.f103y;
        interpolatedOut.f102x = ((a.f102x * u) + (barycentric.f102x * b.f102x)) + (barycentric.f103y * c.f102x);
        interpolatedOut.f103y = ((a.f103y * u) + (barycentric.f102x * b.f103y)) + (barycentric.f103y * c.f103y);
        return interpolatedOut;
    }

    public static float fromBarycoord(Vector2 barycentric, float a, float b, float c) {
        return ((((1.0f - barycentric.f102x) - barycentric.f103y) * a) + (barycentric.f102x * b)) + (barycentric.f103y * c);
    }
    ...
    public static Vector2 triangleCentroid(float x1, float y1, float x2, float y2, float x3, float y3, Vector2 centroid) {
        centroid.f102x = ((x1 + x2) + x3) / 3.0f;
        centroid.f103y = ((y1 + y2) + y3) / 3.0f;
        return centroid;
    }
    ...
    public static Vector2 quadrilateralCentroid(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4, Vector2 centroid) {
        ...
        centroid.f102x = avgX1 - ((avgX1 - (((x1 + x4) + x3) / 3.0f)) / 2.0f);
        centroid.f103y = avgY1 - ((avgY1 - avgY2) / 2.0f);
        return centroid;
    }
    ...
    public static Vector2 polygonCentroid(float[] polygon, int offset, int count, Vector2 centroid) {
        ...
        if (signedArea == 0.0f) {
            ...
            signedArea *= 0.5f;
            centroid.f102x = x / (6.0f * signedArea);
            centroid.f103y = y / (6.0f * signedArea);
        }
        return centroid;
    }
